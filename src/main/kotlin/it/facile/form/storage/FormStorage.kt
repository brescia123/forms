...
class FormStorage(defaultEntries: Map<String, Entry>) {
    ...
    fun setVisibility(key: String, hidden: Boolean, userMade: Boolean = false) {
    ...
    fun putPossibleValues(key: String, possibleValues: FieldPossibleValues, userMade: Boolean = false) {
        if (possibleValues == possibleValuesMap[key]) return // No changes
        possibleValuesMap.put(key, possibleValues)
        clearValue(key, userMade)
    }

    /** Switch possible values for a particular key, switch the associated value and notify the change
     * If at the given key the possible values already present are equal the given ones it does nothing. */
    fun switchPossibleValues(key: String, possibleValues: FieldPossibleValues, userMade: Boolean = false) {
        if (possibleValues == possibleValuesMap[key]) return // No changes
        possibleValuesMap.put(key, possibleValues)
    ...
    fun putValueAndSetVisibility(key: String, value: FieldValue, hidden: Boolean, userMade: Boolean = false) {
    ...
    fun clearPossibleValues(key: String, userMade: Boolean = false) {
